...
export class System {
  ...
  destroy() {
    for (const component of this.registeredComponents.keys()) {
      this.deregisterComponent(component)
    }
  }

  private registerComponent(component: Component<unknown>) {
  ...
  private deregisterComponent(component: Component<unknown>) {
   ...
   */
  protected *getMatchedEntities(): Generator<Entity, void, unknown> {
